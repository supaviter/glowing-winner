...
req_url = r"R:\Development_NII\CRPD_Auto\Ver1\Processor\Config.xlsx"
...
def simulate(timestampStr, prev_date, today_date, download_path):
	...
	down_url1 = rpt_p1_OBD+ptnr_OBD+rpt_p2+prev_date+rpt_p3+today_date+rpt_p4_OBD
	print('\n', down_url1)
	driver.get(down_url1)
	time.sleep(10)
	shutil.move(mv_fm_OBD, mv_to_OBD+timestampStr+'.csv')
	...
	body = driver.find_element_by_tag_name("body")
	body.send_keys(Keys.CONTROL + 't') # Performing CTRL + T Key operation
	down_url2 = rpt_p1_ERA+ptnr_ERA+rpt_p2+prev_date+rpt_p3+today_date+rpt_p4_ERA
	print(down_url2)
	...
	shutil.move(mv_fm_ERA, mv_to_ERA+timestampStr+'.csv')
